Public Class Korisnik
    ...
    Private Sub Korisnik_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'TODO: This line of code loads data into the 'DataSet5.UserRadiliste' table. You can move, or remove it, as needed.
        Me.UserRadilisteTableAdapter.Fill(Me.DataSet5.UserRadiliste)
        'TODO: This line of code loads data into the 'DataSet5.Radiliste' table. You can move, or remove it, as needed.
        Me.RadilisteTableAdapter.Fill(Me.DataSet5.Radiliste)
        ...
        'Prikaži dodijeljene odjele za korisnika NAKON što je dobio UserId
        Dim ta1 As New DataSet5TableAdapters.UserRadilisteTableAdapter
        Dim dt1 As New DataSet5.UserRadilisteDataTable
        '
        For i As Integer = 0 To RadilisteDataGridView.RowCount - 1
            Dim r As DataSet5.RadilisteRow = DirectCast(RadilisteDataGridView.Rows(i).DataBoundItem.row, DataSet5.RadilisteRow)
            If ta1.IsAssociated(UId, r.ID) Then RadilisteDataGridView.Rows(i).Cells(0).Value = True
        Next
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        'Ažuriranje poveznica dodijeljenih odjela

        For i As Integer = 0 To RadilisteDataGridView.RowCount - 1
            Dim r As DataSet5.RadilisteRow = DirectCast(RadilisteDataGridView.Rows(i).DataBoundItem.Row, DataSet5.RadilisteRow)
            Dim ta As New DataSet5TableAdapters.UserRadilisteTableAdapter
            If Convert.ToBoolean(RadilisteDataGridView.Rows(i).Cells(0).Value) Then
                If ta.IsAssociated(UId, r.ID) = 0 Then
                    Dim ur_dt As New DataSet5.UserRadilisteDataTable
                    Dim ur_r As DataSet5.UserRadilisteRow
                    ur_r = ur_dt.NewUserRadilisteRow
                    ur_r.userid = UId
                    ur_r.radiliste = r.ID
                    ur_dt.AddUserRadilisteRow(ur_r)
                    ta.Update(ur_dt)
                End If
            Else
                ta.DeleteQuery(UId, r.ID)
            End If
        Next
